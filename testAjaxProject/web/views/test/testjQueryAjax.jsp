...
<script type="text/javascript">
	$(function(){
		$("#test3").click(function(){
			...
			//$.post() 사용시
			$.post("/testa/t3.do", {title : $("#title").val(), writer : $("#writer").val(), content : $("#content").val() });
			
			//$.ajax 사용시
			$.ajax({
				url: "/testa/t3.do",
				data: {title : $("#title").val(), writer : $("#writer").val(), content : $("#content").val() },
				type: "post"
			});
	...
	$(function(){
		$("#test4").click(function(){
			...
			$.ajax({
				...
				dataType: "json",
				success: function(data){
					$("#p4").html($("#p4").text() + "<br>" + data.no + ", " + data.userid + ", " + data.userpwd + ", " + decodeURIComponent(data.username)
							 + ", " + data.age + ", " + data.email + ", " + data.phone);
				}
			});
...
<hr>
<h2>5. 서버로 값 보내고, 결과로 list 나 map 을 json 배열로 받아 출력 처리하는 방법 : get 방식</h2>
			...
			$.getJSON("/testa/t5.do", {no: $("#no2").val()},
					function(data){
						...
						for(var i in json.list){
							...
							values += json.list[i].userid + ", " + json.list[i].userpwd + ", " + decodeURIComponent(json.list[i].username)
